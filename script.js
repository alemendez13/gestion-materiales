document.addEventListener('DOMContentLoaded', () => {
    ...
    const showAppForUser = (profile) => {
        ...
        // --- LÓGICA DE OBTENCIÓN DE ROL (NUEVA) ---
    // Usaremos leer-catalogo como función de entrada simple para validar la API Key y el Email.
    const fetchRole = async (email) => {
        try {
            const response = await fetch('/.netlify/functions/leer-catalogo', {
                method: 'POST',
                headers: { 
                    'x-api-key': APP_API_KEY, 
                    'Content-Type': 'application/json' 
                },
                // El backend valida el email para determinar el rol.
                body: JSON.stringify({ userEmail: email })
            });

            // Asumimos que el backend devuelve un mensaje de error o éxito.
            // Si la llamada es exitosa (200), asumiremos un 'user' genérico o un 'admin' si tiene acceso.
            // NOTA: El rol real se debe obtener del cuerpo de la respuesta si el backend fuera modificado para devolverlo.
            // Por simplicidad, aquí haremos la verificación manual para las vistas:
            
            // Para las vistas iniciales, el rol debe ser 'admin' o 'user'.
            // Si eres admin, puedes ver el reporte. Haremos una llamada específica:
            const adminCheck = await fetch('/.netlify/functions/generar-reporte', {
                method: 'POST',
                headers: { 'x-api-key': APP_API_KEY, 'Content-Type': 'application/json' },
                body: JSON.stringify({ userEmail: email })
            });

            if (adminCheck.status === 200) return 'admin';
            if (adminCheck.status === 403) return 'user'; // Acceso denegado como admin, pero API Key es válida.

            // Si falla completamente y no devuelve 200 o 403, el usuario no existe.
            return null; 
